...
app.controller('homeController', function ($scope, $routeParams, $location, appService){
            ...
            $scope.$apply( () =>{
                if (!response.error) {
                    if (response.singleUser) {
                        // Removing duplicate user from chat list 
                        ...
                        // Removing a user from chat list, if user offline
                        $scope.data.chatlist = $scope.data.chatlist.filter(function (obj) {
                            return obj.socketid !== response.socketId;
                        });
                    } else {
                        // Update chatlist if user logs in
    ...
    $scope.selectFriendToChat = (friendId) => {
        ...
        // HTTP call will fetch chat between two users
    ...
    $scope.sendMessage = (event) => {
        ...
        if (event.keyCode === 13) {
            ...
            // Fetching the selected User from the chat list 
            ...
            //Emmiting socket event to server with Message
